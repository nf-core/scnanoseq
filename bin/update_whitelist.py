...
def parse_arg():
    ...
    parser.add_argument("putative_bc_csv", type=str, help="Filename of the putative_bc csv file output from blaze.py")
    ...
    requiredNamed.add_argument("--count-threshold", type=int, help="Output the whitelist in Cellranger style")
...
def main(args):
    ...
    for df in tqdm(dfs, desc="Counting high-quality putative BC"):
        raw_bc_count += Counter(df[df.putative_bc_min_q >= args.minQ].putative_bc.value_counts().to_dict())
