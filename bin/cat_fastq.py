...
def parse_args():
    ...
    parser.add_argument(
        "--single_end",
        action="store_true",
        help="Input files are single-end."
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        required=True,
        help="List of input FASTQ files."
    )
...
def main():
    ...
    if args.single_end:
        ...
        if len(read_paths) == 1:
            # If there's only one file, we can just symlink it to the output location instead of copying
        ...
        if len(read_paths) == 2:
            # If there's only one set of files, we can just symlink them to the output location instead of copying
