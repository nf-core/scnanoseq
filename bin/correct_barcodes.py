...
def parse_args():
    ...
    parser.add_argument("--max_edit_dist", default=1, type=int,
                        required=False, help="The maximum edit distance that " \
                                             "a barcode can be from a " \
                                             "barcode on the whitelist")

    parser.add_argument("--min_post_prob", default=0.975, type=int,
...
def correct_bam(infile, outfile, whitelist, barcode_count_file,
                write_filtered_reads, min_post_prob, max_edit_dist):
    ...
    with ps.AlignmentFile(infile, "rb") as bam_in, \
        ps.AlignmentFile(outfile, "wb", template=bam_in) as bam_unfilt_out, \
        ps.AlignmentFile("filtered.bam", "wb", template=bam_in) as bam_filt_out:
        ...
        for read in bam_in:
            ...
            corrected_bc = get_read_bc(read.get_tag('CR'),
                                       read.get_tag('CY'),
                                       whitelist_trie,
                                       bc_probabilities,
                                       max_edit_dist,
            ...
            if corrected_bc:
                ...
                read.query_name = '_'.join([read.query_name.split('_')[0],
                                           corrected_bc,
...
def get_read_bc(barcode, barcode_qual, whitelist_trie, bc_probabilities,
                max_edit_dist, min_post_prob):
    ...
    if not whitelist_trie.has_key(barcode):
        similar_bcs  = get_similar_bcs(barcode, barcode_qual,
                                       whitelist_trie, bc_probabilities,
                                       max_edit_dist)
        if similar_bcs:
...
def get_similar_bcs(query_bc, query_bc_qual, bc_trie, bc_probabilities,
    ...
    for cell_bc in list(get_mutated_bcs(query_bc, max_edit_dist)):
        if bc_trie.has_key(cell_bc):
            bc_probability = get_bc_probability(query_bc, query_bc_qual,
                                                cell_bc, bc_probabilities)
...
def get_mismatch_locs(query_bc, potential_bc):
    ...
    """
    mismatch_locs = []
    for index, bc_chars in enumerate(zip(query_bc,
                                         potential_bc)):
...
def main():
    """ Main Subroutine """
    args = parse_args()
    correct_bam(args.infile, args.outfile, args.whitelist,
                 args.barcode_count, args.write_filtered_reads,
                 args.min_post_prob, args.max_edit_dist)
