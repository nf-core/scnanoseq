...
def parse_args():
    ...
    parser.add_argument("-b", "--in_bam", default=None, type=str, required=True, help="The input bam file")

    parser.add_argument("-f", "--in_fastq", default=None, type=str, required=True, help="The input R1 fastq file")
...
def read_r1_fastq_info(in_fastq, bc_length, umi_length):
    ...
    with pysam.FastxFile(in_fastq) as fh_in_fastq:
        for entry in fh_in_fastq:
            ...
            r1_fastq_info[parsed_read_name] = UmiBcRead(
                entry.name, entry.sequence, entry.quality, bc_length, umi_length
            )
...
def main():
    """Main subroutine"""
    args = parse_args()
    tag_bams(args.in_bam, args.in_fastq, args.out_bam, args.barcode_length, args.umi_length)
