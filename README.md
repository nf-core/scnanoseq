...
- Due to the nature of the data this pipeline analyzes, some tools can experience increased runtimes. For the custom tooling, we have leveraged some of the splitting that is done via the `split_amount` param. The `split_amount` parameter will split the fastq into a number of files that each have a number of lines based on the value used for this parameter. As a result, it is important not to set this parameter to be too low as it could cause a large number of files to process. While this value can be highly dependent on the data, good starting point for an analysis would be to set this value to `500000`. If you find that `PREEXTRAC_FASTQ` and `CORRECT_BARCODES` are still taking long amounts of time to run, it would be worth reducing this parameter to `100000` or `50000`, but keeping the value on the order of hundred of thousands or tens of thousands should help with with keeping the total number of processes minimal.
